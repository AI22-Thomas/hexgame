    ...
    def learn(self, num_episodes=500, batch_size=100, gamma=0.99, eps_start=0.9, eps_end=0.01, eps_decay=1000,
        ...
        for i_episode in range(num_episodes):
            ...
            for t in count():
                ...
                if not terminated:
                    ac2 = self._eps_greedy_action(torch.tensor(observation, dtype=torch.float32, device=self.device).unsqueeze(0), eps=eps_by_step(steps_done))
                    obs2, rew2, terminated2, truncated2 = self.env.step(ac2.item())
                    observation = obs2
                    reward = rew2
